    ...
    return (
        <section id="helhetsanalys" className="col-span-12 scroll-mt-20" data-animate>
            ...
            <div className="space-y-6" data-animate-child>
                {/* Executive Summary */}
                <div className={`relative rounded-2xl border p-7 overflow-hidden ${isBull ? 'border-emerald-500/20 bg-emerald-500/[0.03]' : isBear ? 'border-red-500/20 bg-red-500/[0.03]' : 'border-[#43413A] bg-[#2A2927]'}`}>
                    ...
                    <div className="flex items-start gap-5 pl-4">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-3">
                                <span className="text-[11px] font-700 tracking-[0.12em] uppercase text-[#A6A296]">Overall Assessment</span>
                                <span className={`badge ${isBull ? 'badge-bull' : isBear ? 'badge-bear' : 'badge-hold'}`}><div className="badge-dot" />{signalWord}</span>
                                <span className="font-mono text-[12px] text-[#A6A296]">{score.toFixed(1)}/100 · {conf.toFixed(0)}% conf.</span>
                            </div>
                            <p className="text-[15px] leading-[1.8] text-[#C7C3B8] font-400">{execSummary}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {dimensions.map(({ icon: Icon, label, text, color, bg, border }) => (
                        <div key={label} className={`rounded-xl border p-5 ${bg} ${border} flex flex-col gap-3`} data-animate-child>
                            ...
                            <p className="text-[13px] leading-[1.75] text-[#A6A296]">{text}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="card p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5">
                    ...
                    <div className="w-px h-10 bg-[#3A3833] hidden sm:block shrink-0" />
                    <div className="text-[13px] leading-[1.75] text-[#A6A296]">
