...
import { Activity, BarChart2, Bot, Briefcase, DollarSign, Globe, Shield, Zap, Award } from 'lucide-react';
import { formatRecommendation } from './utils';
...
interface ComprehensiveAnalysisProps {
    ...
    insiderStats: { buyVolume: number; sellVolume: number; netVolume: number };
    insiderTrades: any[];
    riskProfile: any;
...
export default function ComprehensiveAnalysis(props: ComprehensiveAnalysisProps) {
    const { recommendationScore, fundamentalData, technicalIndicators, quantMetrics, macroIndicators,
            gaussianData, manipulationStats, insiderStats, insiderTrades, riskProfile,
            derivedSignal, sig, activeRec, targetKeyword, fmt } = props;

    if (!recommendationScore && !fundamentalData && !technicalIndicators) return null;
    ...
    if (fundamentalData) {
        ...
        const peText = pe ? (pe > 35 ? `P/E of ${pe.toFixed(1)}x indicates a high valuation vs. historical averages, requiring sustained earnings growth to justify the price.` : pe < 12 ? `P/E of ${pe.toFixed(1)}x is low and may signal undervaluation or uncertainty around future earnings.` : `P/E of ${pe.toFixed(1)}x falls within a reasonable range relative to the sector.`) : '';
        const capText = cap ? `Market cap is $${(Number(cap)/1e9).toFixed(1)}B, placing the company in the ${Number(cap)>200e9 ? 'megacap' : Number(cap)>10e9 ? 'large-cap' : 'mid-cap'} category.` : '';
        const analystText = cons ? `Analyst consensus points to ${formatRecommendation(cons)}${mean ? ` with a price target of $${Number(mean).toFixed(2)}` : ''}${low && high ? ` (range $${Number(low).toFixed(0)}–$${Number(high).toFixed(0)})` : ''}.` : '';
    ...
    if (technicalIndicators) {
        ...
        const rsiText = rsi != null ? (rsi > 70 ? `RSI of ${rsi.toFixed(0)} is in overbought territory, increasing short-term reversal risk.` : rsi < 30 ? `RSI of ${rsi.toFixed(0)} signals an oversold market and a potential buying opportunity.` : `RSI of ${rsi.toFixed(0)} is neutral and provides no clear directional signal.`) : '';
        const macdText = macdHist != null ? (macdHist > 0 ? 'MACD histogram is positive, confirming upward momentum.' : 'MACD histogram is negative, suggesting fading momentum.') : '';
        const techText = tech ? `Overall technical signal: ${tech === 'BULLISH' ? 'BULLISH – price action supports a positive view' : tech === 'BEARISH' ? 'BEARISH – technical patterns argue for caution' : 'NEUTRAL – no clear direction emerges'}.` : '';
    ...
    if (quantMetrics) {
        ...
        const hurstText = hurst != null ? (hurst > 0.6 ? `Hurst exponent (${hurst.toFixed(2)}) indicates strong trending price dynamics — impulses tend to persist.` : hurst < 0.45 ? `Hurst exponent (${hurst.toFixed(2)}) suggests mean-reverting behavior — price tends to return to the mean.` : `Hurst exponent (${hurst.toFixed(2)}) is near 0.5, suggesting a random walk with no clear trend.`) : '';
        const kellyText = kelly != null ? `Kelly criterion suggests an allocation of ${(kelly*100).toFixed(1)}% of capital given the current risk/reward ratio.` : '';
        const hmmText = hmm != null ? `HMM regime detection classifies the current market as a ${hmm === 1 ? 'bull regime' : 'bear regime'}.` : '';
        const corrText = corr != null && granger != null ? `Sentiment-price correlation is ${corr.toFixed(2)} and the Granger causality test ${granger < 0.05 ? 'confirms statistical significance (p=' + granger.toFixed(3) + ')' : 'shows no significant causality (p=' + granger.toFixed(3) + ')'}.` : '';
        const adfText = adf != null ? (adf ? 'Price series is stationary, supporting statistical models.' : 'Price series is non-stationary — models should use differenced values.') : '';
    ...
    if (macroIndicators) {
        ...
        const vixText = vix != null ? (vix > 30 ? `VIX of ${vix.toFixed(1)} signals elevated market fear and high volatility — risk premiums are elevated.` : vix < 15 ? `VIX of ${vix.toFixed(1)} indicates calm market conditions with low implied volatility.` : `VIX of ${vix.toFixed(1)} is within normal levels.`) : '';
        const fgText = fg != null ? (fg > 70 ? `Fear & Greed Index at ${fg.toFixed(0)} indicates extreme greed in the market, which historically correlates with elevated correction risk.` : fg < 30 ? `Fear & Greed Index at ${fg.toFixed(0)} shows extreme fear — contrarian analysis suggests potential buying opportunities.` : `Fear & Greed Index at ${fg.toFixed(0)} is neutral.`) : '';
        const yieldText = yield_ != null ? `The 10-year Treasury yield at ${yield_.toFixed(2)}% sets the bar for discount rates and ${yield_ > 4.5 ? 'pressures valuation models negatively' : 'offers relative support for equity valuations'}.` : '';
        const etfText = etf != null ? `Sector ETF momentum over the past month is ${(etf*100).toFixed(1)}%, which ${etf >= 0 ? 'supports a positive sector view' : 'suggests sector rotation away from this segment'}.` : '';
    ...
    if (gaussianData || manipulationStats) {
        ...
        const meanText = mean != null ? (mean > 0.5 ? `Sentiment distribution is clearly positive (μ=${mean.toFixed(2)}) with broad consensus among analyzed posts.` : mean < -0.5 ? `Sentiment distribution is negatively skewed (μ=${mean.toFixed(2)}), reflecting widespread pessimism on social media.` : `Sentiment distribution is centered near neutral (μ=${mean.toFixed(2)}) with divided opinions.`) : '';
        const stdText = std != null ? (std > 0.4 ? `Dispersion is high (σ=${std.toFixed(2)}), indicating diverging opinions — the market is undecided.` : `Dispersion is low (σ=${std.toFixed(2)}), suggesting a consensus view.`) : '';
        const qualText = total != null && organic != null ? `A total of ${total.toLocaleString()} posts were analyzed, of which ${((organic/total)*100).toFixed(0)}% were classified as organic signals${blocked ? ` and ${blocked} manipulative posts were filtered out` : ''}.` : '';
    ...
    if (riskProfile) {
        ...
        const sharpeText = sharpe != null ? (sharpe > 1.5 ? `Sharpe ratio of ${sharpe.toFixed(2)} is excellent, indicating high risk-adjusted returns.` : sharpe > 0.5 ? `Sharpe ratio of ${sharpe.toFixed(2)} is acceptable.` : `Sharpe ratio of ${sharpe.toFixed(2)} is low — returns do not justify the volatility.`) : '';
        const varText = var_ != null ? `Value-at-Risk (95%) is ${(var_*100).toFixed(1)}% daily loss level.` : '';
        const ddText = dd != null ? `Maximum drawdown is ${(Math.abs(dd)*100).toFixed(1)}%, which ${Math.abs(dd) > 0.3 ? 'is significant and should be considered in position sizing' : 'is within acceptable levels'}.` : '';
        const liqText = liq != null ? `Liquidity score is ${liq.toFixed(2)} — ${liq > 0.7 ? 'high liquidity facilitates easy entry and exit' : liq > 0.4 ? 'liquidity is sufficient' : 'low liquidity may result in slippage on larger positions'}.` : '';
