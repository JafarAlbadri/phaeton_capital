...
const SIGNAL_CFG: Record<string, { word: string; color: string; bg: string; border: string }> = {
    STRONG_BUY: { word: "STRONG BUY", color: "#7FA886", bg: "rgba(78,125,83,0.10)", border: "rgba(78,125,83,0.30)" },
    BUY:        { word: "BUY",        color: "#7FA886", bg: "rgba(78,125,83,0.06)", border: "rgba(78,125,83,0.20)" },
    HOLD:       { word: "HOLD",       color: "#D97757", bg: "rgba(201,100,66,0.06)", border: "rgba(201,100,66,0.20)" },
    SELL:       { word: "SELL",       color: "#D9776B", bg: "rgba(194,78,66,0.06)",  border: "rgba(194,78,66,0.20)" },
    STRONG_SELL:{ word: "STRONG SELL",color: "#D9776B", bg: "rgba(194,78,66,0.10)",  border: "rgba(194,78,66,0.30)" },
};
...
export default function DiscoverClient({ horizon, results, universeSize }: Props) {
    ...
    return (
        <main className="relative z-10 min-h-screen px-4 sm:px-6 py-8 max-w-6xl mx-auto">
            {/* Back link */}
            <Link href="/" className="inline-flex items-center gap-1.5 text-[12px] text-[#7878a0] hover:text-[#B3AFA3] transition-colors mb-6">
                ...
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                            ...
                            <Compass size={20} style={{ color: "#E0906F" }} />
                        </div>
                    ...
                    <p className="text-[13px] text-[#7878a0] max-w-xl">
                        Top opportunities surfaced from the tracked universe of <span className="text-[#B3AFA3] font-semibold">{universeSize}</span> stocks.
                        Ranked by composite score across all factors. Re-scanned every 6 hours.
                    </p>
                </div>

                {/* Horizon switcher */}
                <div className="flex gap-1 bg-[#0a0a1c] border border-[#43413A] rounded-lg p-1">
                    {[15, 30, 90].map(h => (
                        <Link
                            key={h}
                            ...
                            className={`px-3 py-1.5 rounded-md text-[12px] font-semibold transition-all ${
                                h === horizon
                                    ? "bg-[rgba(201,100,66,0.15)] text-[#E0906F] border border-[rgba(201,100,66,0.30)]"
                                    : "text-[#7878a0] hover:text-[#B3AFA3]"
                            }`}
                        >
                            {h}d
                        </Link>
            ...
            {results.length === 0 ? (
                <div className="card p-12 text-center" data-animate>
                    ...
                    <p className="text-[#A6A296] font-medium mb-1">No opportunities surfaced yet.</p>
                ...
                <div className="card overflow-hidden" data-animate>
                    {/* Header row */}
                    <div className="hidden sm:grid grid-cols-[80px_1fr_120px_90px_100px_80px_80px] gap-3 px-5 py-3 border-b border-[#43413A] section-title">
                        <div>Ticker</div>
                        <div>Name / Sector</div>
                        <div>Signal</div>
                    ...
                    {/* Rows */}
                    <div className="divide-y divide-[#3A3833]">
                        {results.map((r, i) => {
                            const s = sig(r.signal);
                            const dscore = r.top_driver.score;
                            const driverColor = dscore > 60 ? "#7FA886" : dscore < 40 ? "#D9776B" : "#D97757";
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-mono font-bold text-[15px] text-[#F0EEE6] group-hover:text-[#E0906F] transition-colors">{r.ticker}</span>
                                    </div>

                                    {/* Name / sector */}
                                    <div className="hidden sm:block min-w-0">
                                        {r.name && <div className="text-[13px] text-[#B3AFA3] truncate">{r.name}</div>}
                                    ...
                                    {/* Top driver */}
                                    <div className="hidden sm:block">
                                        <div className="text-[11px] text-[#A6A296]">{DRIVER_LABELS[r.top_driver.factor] ?? r.top_driver.factor}</div>
                                    ...
                                    <div className="text-right hidden sm:block">
                                        {r.hit_rate_pct != null
                                            ? <span className="font-mono text-[12px]" style={{ color: r.hit_rate_pct >= 55 ? "#7FA886" : "#A6A296" }}>{r.hit_rate_pct}%</span>
                                            : <span className="text-[11px] text-[#8F8C80]">–</span>}
                                    </div>

                                    {/* Confidence */}
                                    <div className="text-right hidden sm:block">
                                        <span className="font-mono text-[12px] text-[#A6A296]">{r.confidence_pct}%</span>
                                    </div>
...
function StatTile({ icon: Icon, label, value, variant }: { icon: typeof Target; label: string; value: string; variant: "bull" | "gold" }) {
    const color = variant === "bull" ? "#7FA886" : "#E0906F";
